...
Check whether this PermissionScope's scope includes the scope of the passed PermissionScope. Scope A is considered to be contained within scope B if scope A and scope B's tiers are all equal, or if any of scope B's tiers are wildcards ("*"). In the event that scope B's last tier is a wildcard, all lower tiers are also included (e.g., the scope `sample:*` also contains `sample:*:*`, `sample:*:*:*`, etc.).
